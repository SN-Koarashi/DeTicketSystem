...
- 在 Cloudflare D1 Dashboard 建立資料庫與必要的資料表：`backend/hono/d1_database.sql`。
- 本地開發可直接使用 `npm run init:db` 來初始化本機 D1 資料庫(執行前需要切換到 `backend/hono`)。
